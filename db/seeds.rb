if Rails.env.development?
  ...
  story = project.stories.create! :title => "A user should be able to create features",
    :story_type => 'feature', :requested_by => user, :labels => 'features', :acting_user => user
  project.stories.create! :title => "A user should be able to create bugs",
    :story_type => 'bug', :requested_by => user, :labels => 'bugs', :acting_user => user
  project.stories.create! :title => "A user should be able to create chores",
    :story_type => 'chore', :requested_by => user, :labels => 'chores', :acting_user => user
  project.stories.create! :title => "A user should be able to create releases",
    :story_type => 'release', :requested_by => user, :labels => 'releases', :acting_user => user
  project.stories.create! :title => "A user should be able to estimate features",
    :story_type => 'feature', :requested_by => user, :estimate => 1, :labels => 'estimates,features', :acting_user => user

  2.times do |n|
    note = story.notes.build :note => "This is comment number #{n + 1}"
    note.user = user
    note.story.acting_user = user
    note.save!
  end
end
